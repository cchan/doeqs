...
//class DB //Database access class, which both makes it securer (replacement + checking) and don't have to specify everything each time opening connection.
	...
	//function query($template,$replace1,$replace2,...) //Submits a query, checking all replaceN for SQL validity and then replacing all occurrences of %N% in $templaet with replaceN. If any of replaceN have strings like %4% in them... just go away
...
class DB{
	...
	public function query($template/*, $replace1, $replace2, ..., $replaceN*/){//Is it safe if you use real_escape_string?
		$args=func_get_args();
		...
		if(($qresult=$this->con->query($template))===false)throw new Exception("DB: query failed");
		$this->insert_id=$this->con->insert_id;
		return $qresult;//remember to free results! //what if automatically convert to arr, or some convenience like that?
	}
};
...
function elemInSQLReq($elem,$col,$table){//Checks whether a specified element is in the specified column in the specified database.
	...
	$con=new DB();
	$qresult=$con->query("SELECT %2% FROM %1% WHERE %2%=\"%3%\"",$table,$col,$elem);//Select all from $table where value of $col is $elem.
	unset($con);
	return (($qresult->num_rows)>0);//If there's one or more rows, it is in the column.
}
